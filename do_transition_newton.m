function [paths, statst]  = do_transition_newton(opt, glob, p, init_state, final_ss)
    ...
    res_ss_vec              = [res_ss.K; res_ss.piP; res_ss.H];
    ...
    for t = 1:p.Nt
        if opt.debug_trans >= 1 && mod(t, 10) == 0
            disp(['Calculating Jacobian: ', int2str(1*p.Nt +t) '/' num2str(3 * p.Nt)  ]);
        end
        guesses0.piPt        = zeros(p.Nt, 1);
        ...
        res                 = transition_residuals(opt, glob, p, guesses0, init_state, final_ss);
        res_vec             = [res.K; res.piP; res.H];
        jac_for(:, 1 * p.Nt + t)       = (res_vec - res_ss_vec) / opt.steppiP_nt;
    end
    ...
    for t = 1:p.Nt
        ...
        res                 = transition_residuals(opt, glob, p, guesses0, init_state, final_ss);
        res_vec             = [res.K; res.piP; res.H];
        jac_for(:, 2 * p.Nt + t)       = (res_vec - res_ss_vec) / opt.stepH_nt;
    end
    guesses0.Ht       = ones(p.Nt, 1) * final_ss.H;
    ...
    trans_path              = [ones(p.Nt, 1) * final_ss.K; zeros(p.Nt,1); ones(p.Nt, 1) * final_ss.H];
    fval                    = f(struct('Kt', trans_path(1:p.Nt),...
                                       'piPt', trans_path(1*p.Nt+1:2*p.Nt),...
                                       'Ht', trans_path(2*p.Nt+1:3*p.Nt)));
    fval_vec                = [fval.K; fval.piP; fval.H];
    for it = 1:opt.maxittrans_nt
        ...
        fold_vec            = fval_vec;
        fval                = f(struct('Kt', trans_path(1:p.Nt),  'piPt', trans_path(1*p.Nt+1:2*p.Nt), 'Ht', trans_path(2*p.Nt+1:3*p.Nt)));
        fval_vec            = [fval.K;  fval.piP; fval.H];
    ...
    paths.piPt              = trans_path(1*p.Nt+1:2*p.Nt);
    paths.Ht                = trans_path(2*p.Nt+1:3*p.Nt);
        
    end    
    paths.Kt                = trans_path(1:p.Nt);
    paths.piPt              = trans_path(1*p.Nt+1:2*p.Nt);
    paths.Ht                = trans_path(2*p.Nt+1:3*p.Nt);
    [~, statst]             = f(struct('Kt', trans_path(1:p.Nt),  'piPt', trans_path(1*p.Nt+1:2*p.Nt),  'Ht', trans_path(2*p.Nt+1:3*p.Nt)));
end
function [statst] = calc_distr_irf(opt, glob, p, statst, statst_benchmark)
...
%% THERE IS A BUG RELATED TO THE INITIAL DISTRIBUTION SHIFT!
    ...
    % need to align the grid and the distribution closer
     [a_percentiles_pre] = prctilew(p.a, statst_benchmark{1}.a_marg, [0.0001 0.1 0.25 0.5 0.75 0.9 1]);
     [b_percentiles]     = prctilew(p.b, statst_benchmark{1}.b_marg, [0.0001 0.1 0.25 0.5 0.75 0.9 1]);
     ...
     a_weight_left  = (a_percentiles - a_rep(1,a_ind_left))./(a_rep(1,a_ind_right) - a_rep(1,a_ind_left));
     ...
     a_pre_weight_left  = (a_percentiles_pre - a_rep(1,a_pre_ind_left))./(a_rep(1,a_pre_ind_right) - a_rep(1,a_pre_ind_left));
    ...
    illiq_inc_vec           = reshape(statst{1}.illiq_inc, p.Nb * p.Na * p.Nz, 1);
     ...
     % do the impact adjustment
     
     % SOMETHING WRONG WITH INITIAL DISTRIBUTIONS!!!!!!!
    ...
    for t = 1:p.Nt-1
        ...
        for nz = 1:p.Nz
            ...
            gtilde_pre{t + 1}(:, nz)    = statst_benchmark{t}.B{nz} \ vec;

...
function vResidual = equilibrium_conditions_N(vars)
...
q_K =  vars(2*I*J*P+4) + varsSS(2*I*J*P+4);
r_plus = vars(2*I*J*P+5) + varsSS(2*I*J*P+5);
...
K_dot =vars(nVars+2*I*J*P+3);
q_K_dot =vars(nVars+2*I*J*P+4);
...
savings = c0 - c;
       ...
       %Now use a lopp
        ...
        VDot(((p-1)*I*J+1):p*I*J,1);
        VEErrors(((p-1)*I*J+1):p*I*J,1) - rho * V_stacked;
...
%residual_lambda = lambda - max(0,f/alpha_hat + alpha_hat_dot/alpha_hat - rho_B - f + r_plus); % possibly redundant
residual_K = K_dot - a1*(1-xi)*(Inv/K)^(1-xi)*K + delta*K;
residual_clearing = q_K*K + B_minus - B_plus - N;
residual_assets = B_plus - (leverage-1)*N;
tfpResidual = logAggregateTFPDot + (1 - rrhoTFP) * logAggregateTFP - ssigmaTFP * 0;
vResidual=[residual_hjb;gResidual;tfpResidual;residual_alpha_hat;residual_N;residual_K;residual_clearing;residual_assets];

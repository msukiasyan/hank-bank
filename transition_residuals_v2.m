function [res, statst] = transition_residuals_v2(opt, glob, p, guesses, init_state, final_ss)
    %% Preallocation
    statst          = cell(p.Nt, 1);
    BUt             = cell(p.Nt, p.Nz);
    Vt              = cell(p.Nt, 1);
    ...
    Ygrowtht      = [(Yt(2:end)-Yt(1:end-1))./(p.dt(1:end-1).*Yt(1:end-1));0];
    piPdott    = [(piPt(2:end)-piPt(1:end-1))./p.dt(1:end-1);0];
    
    invmarkupPt      = 1/p.markupP + p.sigmaP / p.zetaP * ((r_plust - Ygrowtht) .* piPt - piPdott);
    markupPt         = 1./invmarkupPt;
    %%  marginal product and factor prices
    mpk              = prod_K(opt, glob, p, Kt, Ht);
    
    r_minust1(p.Nt) = final_ss.r_minus;
    
    for t = 1:p.Nt-1
        r_minust1(t) = (mpk(t)/markupPt(t) - iotat(t) + (qt(t + 1) - qt(t)) / p.dt(t)) / qt(t) + Psit(t) - p.delta;
    end
